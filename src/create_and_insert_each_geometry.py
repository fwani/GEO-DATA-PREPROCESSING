...
def run(args):
    ...
    si_file = sorted(glob.glob(os.path.join(home, 'data/shape_each/CTP*/*.shp')))[-1]
    gu_file = sorted(glob.glob(os.path.join(home, 'data/shape_each/SIG*/*.shp')))[-1]
    dong_file = sorted(glob.glob(os.path.join(home, 'data/shape_each/EMD*/*.shp')))[-1]
