...
def run(args):
    ...
    si_file = sorted(glob.glob(os.path.join(home, 'data/shape_each/CTP*/*_simplify.shp')))[-1]
    gu_file = sorted(glob.glob(os.path.join(home, 'data/shape_each/SIG*/*_simplify.shp')))[-1]
    dong_file = sorted(glob.glob(os.path.join(home, 'data/shape_each/EMD*/*_simplify.shp')))[-1]
    print(si_file, gu_file, dong_file)
    ...
    print(dong_df[dong_df['geometry'] == None])
    ...
    cur.execute(open(os.path.join(home, 'sql/create_and_insert_each_jiri_table_simplify.sql'), 'r').read())
