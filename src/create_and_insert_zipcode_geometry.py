...
def run(args):
    ...
    jiri_files = glob.glob(os.path.join(home, 'data/shape/*기초구역DB_전체분/*/*.shp'))
    print(jiri_files)
    ...
    cur.execute(open(os.path.join(home, 'sql/create_and_insert_zipcode_jiri_table.sql'), 'r').read())
    for jiri_file in jiri_files:
        ...
        insert_table(cur, jiri_df, base_insert_sql, '지리_우편번호')
'''
    category_columns = {
